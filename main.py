...
def create_model(args, num_classes):
    model = models.create(args.arch, num_features=args.features, dropout=args.dropout, norm=True,num_classes=num_classes,args=args)
    model.cuda()
...
def main_worker(args):
    ...
    if args.evaluate:
        checkpoint = torch.load('/home/LEDF/logs-main/best.pth.tar')
    ...
    params = [{"params": [value],"lr":args.lr} for value in model.parameters() if value.requires_grad]
    ...
    criterion1 = TripletLoss(args.margin, args.num_instances, False).cuda()
    trainer = Trainer(args, model,memories, criterion1)
    ...
    for epoch in range(args.epochs):
        ...
        trainer.train_ledf(epoch, train_loader, optimizer,print_freq=args.print_freq, train_iters=args.iters)
        if (epoch + 1) % args.eval_step == 0 or (epoch == args.epochs - 1):
            mAP = evaluator.evaluate(test_loader, dataset.query, dataset.gallery, cmc_flag=False)
            is_best = (mAP > best_mAP)
            best_mAP = max(mAP, best_mAP)
            if is_best:
                torch.save(model.state_dict(), osp.join(args.logs_dir, 'best.pth.tar'))
    ...
    checkpoint = torch.load(osp.join(args.logs_dir, 'best.pth.tar'),map_location='cuda:0')
    model.load_state_dict(checkpoint)
    evaluator.evaluate(test_loader, dataset.query, dataset.gallery, cmc_flag=True)
...
if __name__ == '__main__':
    ...
    # data
    #dukemtmc,market1501,cuhk03,cuhknp,msmt17v1


    #--------------------------------------------------------------------------------------
    parser.add_argument('-d', '--dataset', type=str, default='market1501',
                        choices=datasets.names())
    parser.add_argument('--dataset_src1', type=str, default='msmt17',
                        choices=datasets.names())
    parser.add_argument('--dataset_src2', type=str, default='cuhk02',
                        choices=datasets.names())
    parser.add_argument('--dataset_src3', type=str, default='cuhk03',
                        choices=datasets.names())

    parser.add_argument('-b', '--batch-size', type=int, default=64)  #16
    parser.add_argument('--test-batch-size', type=int, default=256)  #128
    ...
    # model
    parser.add_argument('-a', '--arch', type=str, default='resnet50_ledf',
